...
def read_fasta(fasta_file):
	...
	for line in fh.readlines():
		if line[0] == '>':
			ID = line.split('] ')[0]
			entry[ID.split('gene=')[1]] = ''

		else:
			entry[ID.split('gene=')[1]] += line.strip()
...
def main():
	...
	# Define what the subentries are for parallelization
	subentries = read_dict(subset_name)

	# 1) Find all sgRNAs
	now = datetime.datetime.now()
	motif = Motif('motif.txt')
	entries = read_fasta('Synechocystis_ORFs.txt')
	genome = read_genome('Synechocystis_whole_genome.txt')
	print 'screening for a lot of sgRNAs!...'
	results = find_sequence(1, motif, entries, subentries)# // at this point
...
def test():
	...
	now = datetime.datetime.now()
	motif = Motif('motif.txt')
	entries = read_fasta('Synechocystis_ORFs_test.txt')
	genome = read_genome('Synechocystis_whole_genome.txt')
	print 'screening for a lot of sgRNAs!...'
